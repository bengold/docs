---
title: "Concepts"
description: "Helpful concepts to help you become successful when creating Plugins."
---

## Plugin Architecture

A plugin is a small website that runs inside of Framer in a secure iframe and interacts with the Framer editor via the `framer` API. In the case of the plugin template, it is a React application with everything ready to go.
...
The plugin template has Framer styling built-in to many common elements. These styles live in the `globals.css` file, however you shouldn’t need to edit them directly. Custom styling of your plugin can be done in the `App.css` file. Learn more in the [styling guide](/plugins/guides/interface).
